...
@pytest.fixture(scope="session")
def usa_graph() -> nx.DiGraph:
    path = os.path.join(os.path.dirname(__file__), "USA-road-d.USA.gr")
    if not os.path.isfile(path):
        pytest.fail(f"USA graph file not found: {path}")
    t0 = time.perf_counter()
    G = load_dimacs_graph(str(path), directed=True)
    t1 = time.perf_counter() - t0
    print("")
    print(f"USA graph load time = {t1:.3f}s")
    return G
